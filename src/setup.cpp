...
/*void main_setup() { // Poiseuille flow validation
	...
	while(true) {
			...
			for(uint y=Ny/2u; y<Ny/2u+1u; y++) {
				for(uint z=0; z<Nz; z++) {
					...
					if(r<R) {
						...
						error_dif += sq(unum-uref); // L2 error (Kr�ger p. 138)
						error_sum += sq(uref);
		...
		if(sqrt(error_dif/error_sum)>=error_min) { // stop when error has converged
			print_info("Poiseuille flow error converged after "+to_string(lbm.get_t())+" steps to "+to_string(error_min)); // typical expected L2 errors: 2-5% (Kr�ger p. 256)
			wait();
			exit(0);
		}
		error_min = fmin(error_min, sqrt(error_dif/error_sum));
		print_info("Poiseuille flow error after t="+to_string(lbm.get_t())+" is "+to_string(error_min)); // typical expected L2 errors: 2-5% (Kr�ger p. 256)
	}
} /**/
